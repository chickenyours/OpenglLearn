...
vec3 CalDirectBlingPhoneLightWithShadowMap(DirectLight light,Material material,vec3 fragPos,vec2 texCoord,vec3 normal,vec3 viewPos,vec4 fragPosLightSpace);
vec3 CalPointBlingPhoneLightWithCubeShadowMap(PointLight light,Material material,vec3 fragPos,vec2 texCoord,vec3 normal,vec3 viewPos);
//vec3 CalPointBlingPhoneLight(PointLight light,vec3 fragPos,vec3 normal,vec3 viewPos);
...
void main(){
    vec3 color = vec3(0.0);
    //平行光照射
    //color += CalDirectBlingPhoneLightWithShadowMap(dlight,material,fs_in.FragPos,fs_in.TexCoords,fs_in.Normal,vpos,fs_in.LightFragPos); 
...
vec3 CalDirectBlingPhoneLightWithShadowMap(DirectLight light,Material material,vec3 fragPos,vec2 texCoord,vec3 normal,vec3 viewPos,vec4 fragPosLightSpace){
    ...
    float shadow = 0.0;
    for(int i = 0;i<9;i++){
        vec2 offset = offsets[i]/textureSize(shadowMap, 0);
...
vec3 CalPointBlingPhoneLightWithCubeShadowMap(PointLight light,Material material,vec3 fragPos,vec2 texCoord,vec3 normal,vec3 viewPos){
    ...
    for(float x = -offset; x < offset; x += offset / (samples * 0.5))
    {
        for(float y = -offset; y < offset; y += offset / (samples * 0.5))
        {
            ...
            {
                float closestDepth = texture(light.depthMap, fragToLight + vec3(x, y, z)).r; 
