...
class IBPR : public IMaterial{
    ...
    public:
        ...
        bool LoadFromMataData(
            ...
            albedoMap_ = ECS::Core::ResourceModule::ResourceManager::GetInctance().Get<Texture2D>(FromConfig<Texture2D>(albedoMapPath));
            if(!albedoMap_){
                REPORT_STACK_ERROR(errHandle, "Material:BPR", "Failed to load albedoMap resource.");
                return false;
            }
            normalMap_ = ECS::Core::ResourceModule::ResourceManager::GetInctance().Get<Texture2D>(FromConfig<Texture2D>(normalMapPath));
            if(!normalMap_){
            ...
            if(state_.useRoughnessMap){
                roughnessMap_ = ECS::Core::ResourceModule::ResourceManager::GetInctance().Get<Texture2D>(FromConfig<Texture2D>(roughnessMapPath));
                if (!roughnessMap_) {
            ...
            if(state_.useMetallicMap){
                metallicMap_ = ECS::Core::ResourceModule::ResourceManager::GetInctance().Get<Texture2D>(FromConfig<Texture2D>(useMetallicMapPath));
                if (!metallicMap_) {
            ...
            if(state_.useAoMap){
                aoMap_ = ECS::Core::ResourceModule::ResourceManager::GetInctance().Get<Texture2D>(FromConfig<Texture2D>(aoMapPath));
                if (!aoMap_) {
            ...
            mainShader_ = ECS::Core::ResourceModule::ResourceManager::GetInctance().Get<ShaderProgramFactory>(
