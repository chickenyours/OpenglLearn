...
#include "engine/ToolAndAlgorithm/Json/json_config_identification.h"

#include "engine/DebugTool/ConsoleHelp/color_log.h"

#include "engine/Resource/Shader/shader.h"

#include "engine/ToolAndAlgorithm/Hash/md5.h"
...
bool ShaderProgramFactory::LoadFromConfigFile(const std::string& configFile, Log::StackLogErrorHandle errHandle) {
    ...
    if (shaderType_ == ShaderProgramType::VERTEX_FRAGMENT) {
        ...
        auto c = loadShaderFactory(cs, "compute", errHandle);
        if (!c) return false;

        shaderFactories_.emplace(ShaderStage::Compute, std::move(c));
        localShaderDescriptions_.emplace(
            ShaderStage::Compute,
            loadShaderLocalDescription(cs, errHandle)
        );
    }
    else{
