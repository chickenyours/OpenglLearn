...
#include "engine/ECS/Component/Script/script_instance.h"

typedef void(*set_dll_module_api)(const Module::ModuleHost& host);
typedef ECS::IScript*(*get_script_object_api)();
typedef void(*delete_script_object_api)(ECS::IScript*);
...
class ScriptManager{
    public:
        ...
        ECS::IScript* GetScriptObject(std::string scriptName){
            if(!isLoadDLL_){
