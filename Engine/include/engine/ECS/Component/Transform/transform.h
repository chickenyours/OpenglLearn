    ...
    struct Transform : Component<Transform>
    {
        ...
        void CalQuaternion(){
            if (rotationMode == RotationMode::Euler)
            {
                ...
                quaternion = glm::normalize(qy * qx * qz);
            }
        }
        ...
        inline void CalForwardAndYaw(glm::vec3& forward, float& yawDeg)
        {
            ...
            // forward = 世界空间中的局部 +Y 方向
            forward = glm::normalize(glm::rotate(q, glm::vec3(0.0f, 1.0f, 0.0f)));
            ...
            {
                // 当 forward 接近上下方向（与 Y 轴几乎重合）
                // 使用四元数的 yaw 作为稳定 fallback
