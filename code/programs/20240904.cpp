...
int main()
{
    ...
    glm::mat4 model(1.0);
    Opengl::Shader vs(GL_VERTEX_SHADER,"20240908/shader.vs");
    Opengl::Shader fs(GL_FRAGMENT_SHADER,"20240908/shader.fs");
    Opengl::ShaderProgram shaderProgram(&vs,&fs);
    glm::mat4 view;
    glm::mat4 Oil_model =glm::translate(model,glm::vec3(0.0,20.0,0.0)) * glm::scale(model,glm::vec3(0.1));
    glm::mat4 projection = glm::perspective(glm::radians(45.0f),(float)SCR_WIDTH/(float)SCR_HEIGHT,0.1f,500.0f);
    shaderProgram.Use();
    glUniformMatrix4fv(glGetUniformLocation(shaderProgram.getShaderProgramID(),"model"),1,GL_FALSE,glm::value_ptr(Oil_model));
    ...
    glUniform1i(glGetUniformLocation(shaderProgram.getShaderProgramID(),"diffusion"),0);
    
    MyTool::Model m("./models/xm177_with_gl_obj/xm177.obj");
